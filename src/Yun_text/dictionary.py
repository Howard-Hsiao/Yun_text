...
class Dictionary:
    ...
    def centroid(self):
        ...
        for i in range(len(centroid)):
            result[centroid[i].word] = centroid[i].tf * \
                math.log((self._documentNum + 0.1)/(centroid[i].df), 10)
        return result
    ...
    def get_tf_idf(self, src, normalized = True): # !!!add the other tolerant param, do not know is it property
        srcTf = list()
        ...
        for word, tf in srcTf:
            ...
            srcData[word] = {
                ...
                "tf":tf,
                "idf":math.log((self._documentNum+0.1)/(wordData.df), 10),
                "tf-idf": tf * math.log((self._documentNum+0.1)/(wordData.df), 10)
            }
